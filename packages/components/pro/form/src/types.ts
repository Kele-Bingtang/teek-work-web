...
import type { FormItemColumnProps, CommonValueType, ProFormItemEmits } from "@teek/components/pro/form-item";
...
export interface FormColumn<T = Record<string, any>> extends FormItemColumnProps<T> {
   ...
   */
  defaultValue?:
    | MaybeRef<CommonValueType>
    | Promise<CommonValueType>
    | ((params: DefaultValueParams<T>) => CommonValueType | Promise<CommonValueType>);
  /**
   * 表单排序（从大到小）
   */
  order?: number;
