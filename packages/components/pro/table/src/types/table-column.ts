...
import type { ElOption, FormItemColumnProps, RenderTypes } from "@teek/components/pro/form-item";
 ...
 * 表格行 Row
 */
export type TableRow<T extends Record<string, any> = any> = T & {
  /**
   * options 字典枚举
   */
...
export interface TableColumn<T extends Record<string, any> = any>
   ...
   * el 组件的插槽
   */
  elSlots?: { [slotName: string]: (data: TableRenderParams & Record<string, any>) => RenderTypes };
  /**
   * 其他扩展
   */
  [key: string]: any;
