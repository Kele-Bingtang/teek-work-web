...
const submit = () => {
  proFormInstance.value?.elFormInstance?.validate(valid => {
    if (valid) {
      ...
      proFormInstance.value?.elFormInstance?.clearValidate();
    }
  });
};
...
const columns: FormColumn[] = [
  {
    ...
    label: "用户名称",
    el: "el-input",
    elProps: { clearable: true, placeholder: "请输入 用户名称", maxlength: "30" },
  },
  {
    prop: "phone",
    label: "联系方式",
    br: true,
    el: "el-input",
    elProps: { clearable: true, placeholder: "请输入 领导", maxlength: "11" },
  },
  {
    prop: "email",
    label: "用户邮箱",
    el: "el-input",
    elProps: { clearable: true, placeholder: "请输入 用户邮箱", maxlength: "50" },
  },
  {
    prop: "sex",
    label: "用户性别",
...
<template>
  <ProForm ref="proFormInstance" v-model="user" :columns :el-form-props="elFormProps" :not-clean-model-keys="['id']">
    <template #operation>
