...
import { listMyAllTeamRoute } from "@/common/api/team";
import { AddTeamMenu } from "@/views/team-add/index";
...
export const useRouteFn = () => {
  ...
  const initDynamicRoutes = async (roles?: string[], api?: BackendApi) => {
    ...
    if (!routeList.length) {
      ...
      // 后端控制模式
      else if (isBackendMode) routeList = api ? await getDynamicRoutesFromBackend() : [];
      // 前后端控制模式
   ...
   * 从后台接口获取动态路由
   */
  const getDynamicRoutesFromBackend = async () => {
    const res = await listMyAllTeamRoute();
    const teamRoute = res.data || [];
    teamRoute[0].children?.push({
      path: "/add-team",
      name: "AddTeam",
      meta: {
        render: () => AddTeamMenu(),
