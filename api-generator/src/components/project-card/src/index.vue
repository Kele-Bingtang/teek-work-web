...
const ns = useNamespace("project-card");
...
<template>
  <div :class="ns.b()">
    <div :class="ns.e('header')" v-if="!onlyBody" @click="handleClick">
      <slot name="header">projectName</slot>
      <el-icon class="action">
        <Right />
      </el-icon>
    </div>
    <div :class="[ns.e('body'), { 'is-only': onlyBody }]">
      <slot></slot>
    </div>
    <div :class="ns.e('footer')" v-if="!onlyBody">
